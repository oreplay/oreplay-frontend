  ...
  return (
    <Box>
      <FormControl sx={{minWidth: '10em'}} required>
        ...
        >
          {props.isLoading? <MenuItem key={"Loading"} value={"loading"}>{t('Loading')}</MenuItem> : props.classesList?.map(
            (class_item) => {
              return (
                <MenuItem key={class_item.id} value={class_item.id}>{class_item.short_name}</MenuItem>
              )
            }
          )}
        </Select>
